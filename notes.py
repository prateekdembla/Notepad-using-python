...
main_application.title('Notes text editor')
...
file =  tk.Menu(main_menu, tearoff=False)           #tearoff is used so that the file menu doesn't get detached from file menu
...
#color theme
ligth_default_icon = tk.PhotoImage(file='icons2/light_default.png')
...
color_icons = (ligth_default_icon,light_plus_icon,dark_icon,red_icon,monokai_icon,night_blue_icon)
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        # characters = len(text_editor.get(1.0, 'end-1c').replace(' ',''))  #THis can be write for not including spaces in status bar
...
edit.add_command(label='Clear All' , image=clear_all_icon , compound=tk.LEFT , accelerator='Ctrl+shift+X', command=lambda:text_editor.delete(1.0,tk.END))
